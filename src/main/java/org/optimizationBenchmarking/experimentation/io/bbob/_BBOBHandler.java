...
final class _BBOBHandler implements Comparator<Number[]> {
   ...
   */
  _BBOBHandler(final Logger logger, final ExperimentSetContext esb) {
    super();
    this.m_esc = esb;
  ...
  private final void __handleInstance(final Path tdat, final Path dat,
    ...
    if ((this.m_logger != null) && (this.m_logger.isLoggable(Level.FINER))) {
      this.m_logger.finer("Starting to load data from file combo " + //$NON-NLS-1$l
          tdat + " / " + dat); //$NON-NLS-1$
    }
    ...
    if ((this.m_logger != null) && (this.m_logger.isLoggable(Level.FINER))) {
      this.m_logger.finer("Finished loading data from file combo " + //$NON-NLS-1$
          tdat + " / " + dat); //$NON-NLS-1$
    }
  }
  ...
  private final void __handleDataFolder(final int fid, final Path dir,
    ...
    if ((this.m_logger != null) && (this.m_logger.isLoggable(Level.FINER))) {
      this.m_logger.finer("Begin handling data folder " + dir + //$NON-NLS-1$
    ...
    try (DirectoryStream<Path> files = Files.newDirectoryStream(dir)) {
      ...
      data = new Path[BBOBDriver.DIMENSIONS.length][2];
      count = 0;
                  ...
                  if ((ch == 'A') || (ch == 'a')) {
                    ...
                    if ((ch == 'D') || (ch == 'd')) {
                      ...
                      try {
                        ...
                        i = Arrays.binarySearch(BBOBDriver.DIMENSIONS, b);
                        if (i >= 0) {
                          data[i][td ? 0 : 1] = f;
                          count++;
                          continue outer;
          ...
          for (count = data.length; (--count) >= 0;) {
            if ((data[count][0] != null) || (data[count][1] != null)) {
              try (final InstanceRunsContext ic = ec.createInstanceRuns()) {
                ic.setInstance(BBOBDriver._makeFunctionName(fid,
                    BBOBDriver.DIMENSIONS[count]));
    ...
    if ((this.m_logger != null) && (this.m_logger.isLoggable(Level.FINER))) {
      this.m_logger.finer("Finished handling data folder " + dir + //$NON-NLS-1$
          " for function id " + fid); //$NON-NLS-1$
    }
  }
  ...
  private final void __handleDirectory(final Path dir) throws Throwable {
    ...
    subError = null;
    if ((this.m_logger != null) && (this.m_logger.isLoggable(Level.FINER))) {
      this.m_logger.finer(("Now entering folder '" //$NON-NLS-1$
          + dir) + '\'');
    }
    ...
    try (DirectoryStream<Path> files = Files.newDirectoryStream(dir)) {
      ...
      dataFolders = new Path[BBOBDriver.MAX_FUNCTION + 1];
      dataFolderCount = 0;
    ...
    if ((this.m_logger != null) && (this.m_logger.isLoggable(Level.FINER))) {
      this.m_logger.finer(("Now leaving folder '" //$NON-NLS-1$
          + dir) + '\'');
    }
