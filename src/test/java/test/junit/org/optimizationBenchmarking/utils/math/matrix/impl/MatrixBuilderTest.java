...
public class MatrixBuilderTest extends TestBase {
  ...
  /** test building a matrix */
  @Test(timeout = 3600000)
  public final void testFloatWithExtremeCasesMatrixBuilding() {
    final Random rand;
    double[][] data;
    boolean hasM, hasN;
    int testIt, i, j, curType, size, intVal, int2;
        ...
        for (j = 0; j < data[i].length; j++) {
          ...
          switch (rand.nextInt(curType)) {
            ...
            case 4: {
              switch (rand.nextInt(1000)) {
                case 0: {
                  ...
                  data[i][j] = Float.POSITIVE_INFINITY;
                  break;
                }
                case 1: {
            ...
            default: {
              switch (rand.nextInt(1000)) {
                case 0: {
                  builder.append(data[i][j] = Double.POSITIVE_INFINITY);
                  break;
                }
                case 1: {
                  builder.append(data[i][j] = Double.NEGATIVE_INFINITY);
                  break;
                }
                case 2: {
                  builder.append(data[i][j] = Double.NaN);
                  break;
                }
                default: {
                  doubleVal = (rand.nextBoolean() ? (rand.nextLong() / 32d)
